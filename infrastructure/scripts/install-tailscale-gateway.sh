...
# Tailscale Installation Script for Hetzner Gateway VPS
# This script installs and configures Tailscale on the primary gateway node
...
set_node_name() {
    log "Setting node name to 'gateway-hetzner'..."
    ...
    sudo hostnamectl set-hostname gateway-hetzner

    success "Node name configured as 'gateway-hetzner'"
}
...
display_next_steps() {
    ...
    echo "   tailscale ip -4"
    echo
    echo "5. Test connectivity with other nodes once they're set up"
}

# Main execution
main() {
    echo "========================================"
    echo "  Tailscale Gateway VPS Installation"
    echo "========================================"
    echo

    log "Starting Tailscale installation on Hetzner Gateway VPS..."
